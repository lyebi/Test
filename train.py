...
parser.add_argument('-e', '--env-id', type=str, default="MontezumaRevenge-v0",
                    help="Environment id")
parser.add_argument('-l', '--log-dir', type=str, default="/home/liboxing/tmp/amulti_goal_maliaov111",
...
def create_commands(session, num_workers, remotes, env_id, logdir, shell='bash', mode='tmux', visualise=False):
    ...
    cmds_map += [new_cmd(session, "tb", ["tensorboard", "--logdir", logdir, "--port", "12348"], mode, logdir, shell)]
    if mode == 'tmux':
    ...
    notes += ["Point your browser to http://localhost:12347 to see Tensorboard"]

    if mode == 'tmux':
        cmds += [
        "kill $( lsof -i:12347 -t ) > /dev/null 2>&1",  # kill any process using tensorboard's port
        "kill $( lsof -i:12522-{} -t ) > /dev/null 2>&1".format(num_workers+12522), # kill any processes using ps / worker ports
...
def run():
    ...
    cmds, notes = create_commands("a3c2", args.num_workers, args.remotes, args.env_id, args.log_dir, mode=args.mode, visualise=args.visualise)
    if args.dry_run:
